---
title: Render Container 渲染容器
description: Render Container 渲染容器 相关说明与示例。
---
...
:component-example{name="RenderContainerBasic"}
...
你可以使用组件库为你提供的 `容器渲染策略`, 按需引入 [TemplatesContainer](/docs/component/templates/templates-container) 来使用
...
:component-example{name="RenderContainerEcharts"}

:::tip
你可能会遇到 `echarts` 等其他依赖缺失，手动安装即可！

可以查阅 [TemplatesContainer 核心代码](/docs/component/templates/templates-container#templatescontainer-核心代码), 查看可用的渲染策略。但不要忘记传递 `containers` 启用策略哦.
:::
